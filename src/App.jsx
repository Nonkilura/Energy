...
import { Thermometer, Wind, Droplets, AlertTriangle, CheckCircle2, MapPin, RefreshCw, Database } from 'lucide-react';

export default function App() {
  // ⚠️ เปลี่ยน URL ด้านล่างนี้เป็นของ Firebase คุณ (ให้เหมือนกับในไฟล์ Python data_worker.py)
  ...
  // ฟังก์ชันดึงข้อมูลจาก Firebase (ไม่มีปัญหา CORS 100%)
  ...
  return (
    <div className="min-h-screen bg-slate-950 text-slate-200 font-sans flex">
      ...
      <div className="w-72 bg-slate-900 border-r border-slate-800 p-6 flex flex-col shadow-xl z-10">
        ...
        {/* System Status */}
        <div className={`p-4 rounded-xl mb-8 border ${systemStatus.success > 0 ? 'bg-emerald-500/10 border-emerald-500/30' : 'bg-amber-500/10 border-amber-500/30'}`}>
          <div className="flex items-center gap-3 mb-2">
            ...
            <span className={`text-sm font-semibold ${systemStatus.success > 0 ? 'text-emerald-400' : 'text-amber-400'}`}>
              สถานะข้อมูลใน Cloud Database
            </span>
          </div>
          <p className="text-xs text-slate-300">
            อัปเดตล่าสุด: {systemStatus.lastUpdated || "กำลังโหลด..."}<br/>
            ความสมบูรณ์: {systemStatus.success}/{systemStatus.total} สถานี
          </p>
        </div>
        ...
        >
          ...
          {loading ? 'กำลังซิงค์...' : 'อัปเดตจาก Database'}
        </button>
      </div>
      ...
      <div className="flex-1 p-8 overflow-y-auto relative">
        ...
        <div className="mb-4">
          <h3 className="text-lg font-semibold text-slate-200 mb-4 flex items-center gap-2">
            ...
            ข้อมูลโทรมาตรล่าสุด (จาก Firebase)
          </h3>
          ...
          {loading ? (
             <div className="h-64 flex items-center justify-center border-2 border-dashed border-slate-800 rounded-2xl">
                <div className="animate-pulse text-slate-500 flex items-center gap-2">
                  <RefreshCw className="w-5 h-5 animate-spin" /> กำลังโหลดข้อมูล...
                </div>
             </div>
          ) : data.length === 0 ? (
             <div className="h-64 flex flex-col items-center justify-center border border-rose-500/30 bg-rose-500/5 rounded-2xl text-rose-400">
                ...
                <p className="text-xs opacity-70 mt-1">กรุณารันไฟล์ data_worker.py เพื่อส่งข้อมูลขึ้นมา</p>
             </div>
          ) : (
